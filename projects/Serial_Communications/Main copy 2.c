...
void demo_interrupt_commands(void)
{
    ...
    while (1)
    {
        // EDUCATIONAL: Use interrupt buffer to demonstrate non-blocking I/O
        if (!chars_available())
        {
            // CPU is free to do other tasks while waiting for input!
            _delay_ms(10);
            continue;
        }
...
void demo_interrupt_advanced(void)
{
    ...
    while (1)
    {
        // EDUCATIONAL: Use interrupt buffer to show true non-blocking operation
        if (!chars_available())
        {
            // CPU is free - could do other tasks here!
            _delay_ms(5);
            continue;
        }
